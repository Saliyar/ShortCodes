...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function  SPAR_OuterBox_HeaveFD(Exptpath,SPAR_Postprocessing_foamStar,titl,ylbl,nStart,nEnd,lgd3,phaseshift,xlbl)

    %% Non Dimensionalising the Values 
   T_Heave=4.84;
   Heave_amp=0.045;
...
%   motion_Experiment=[pp_linear(1260:end,:) pp_rotation(1260:end,:)];
   ...
   Heave_Experiment=motion_Experiment(Start_index:end,3);%+Heave_amp;
...
for j=nStart:nEnd
   n=n+1;
    ...
    findpeaks(data{:,4},'MinPeakHeight',0.01,'MinPeakDistance',200)
...
for j=nStart:nEnd   
   ...
   plot(dt_motion,Heave_Experiment,'Linestyle','--','LineWidth',3,'color','k')
    ...
    %legend ('Outer Box - Fine','No Outer Box','Experiment','interpreter','latex','FontSize',32);
    legend ('Coarse','Medium','Fine','Very Fine','Experiment','interpreter','latex','FontSize',32);
