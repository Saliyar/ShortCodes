...
for i=1:length(VarDelta)
    for j=1:length(Var_inlet)
        Filepath=fullfile('/mnt','data2','saliyar','Spece_constraint','Files_from_LIGER','foamStar_2D_ParamtericStudy','Relaxationzone_study','Inlet_zone',['foamStar2D_Dx',VarDelta{i},'MeshCo',VarCo{i},'_Inlet_',Var_inlet{j}],'postProcessing','waveProbe','0','surfaceElevation.dat');
...
Time_index=find(data2{:,1}>38.5 & data2{:,1}<40);
...
legend ('Inlet-1B','Inlet-0.5B','Inlet-0.25B','Inlet-0.1B','HOS','FontSize',32);
hold off;
...
plot(rmax(i:i+4),'-o','LineWidth',3)l 
xlim([0.5 5.75])
xticks(1:5)
xticklabels({'Inlet 1B','Inlet 0.5B','Inlet 0.25B','Inlet 0.1B','HOS'})
xtickangle(45)
