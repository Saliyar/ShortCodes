function LHEEA_irrwave_generation_SW_SPAR_16_01_2020
...
% addpath(genpath('/home/saliyar/Documents/Working/LHEEA_Wave_Generation Matlab/utils'))
...
% Repeat period 
rnum = 15; % with corresponding repeat period 12=128s, 14=512s, 15=1024s, 16=2048s, 17=4096s
T_repeat = pow2(rnum-5);
%% .wav file 
file_name   = 'Hs_6_2_Tp_15_0'; % name of the .wav file
file_header = 'Irregular waves for ECN campaign SOFTWIND SPAR'; % will appear in the client.exe windows
file_path  = strcat('SW_SPAR_waves_24_01_2020', filesep); % path relative to the current routine's path where the .wav file will be stored
%% Frequency spectrum
% Significant wave height
% === THE WAVE PARAMETERS INPUT ARE DEFINED HERE !!=======
% additionnal cases
% H_s = [	0.078	0.218	0.273	0.235	0.193	0.055	0.076	0.108	0.155	0.155	0.208	];
% T_p = [	1.385	2.088	1.897	2.182	1.423	1.739	1.897	1.581	1.976	2.372	1.581	];
% H_s  = 0.0650;
% T_p = 1.739;
...
% H_s = [0.020000 0.047500 0.145000 0.192500 0.272500]; % in m
% Wave period at the peak of the spectrum
% T_p = [1.106797 1.233288 1.723441 1.960612 2.371708]; % in s
...
%phase_set(end-2) = 3;
% Target location
target_location = 0; % m (put zero if not relevant)
%
%% Directionality
% Mean direction
...
%% Writing headers
fid = ocean_write_header([file_path, filesep, file_name], file_header, routine, ttf_file);
fid_sum = ocean_write_summary_header([file_path, filesep, file_name], file_header, routine, ttf_file);
%
%% Wave data
...
for m=1:N_spectra
    ...
    % Modif V.A : To make a reproductible result, I'm saving the random
    % phases to be able to generate the same file again.
    if write_phase_file == 1
        phase_file = strcat(file_path,'SW_SPAR_additionnal_DLC_1_2_phase_m', num2str(m), '.mat');
        save (phase_file, 'phase', 'harmo')
    end
