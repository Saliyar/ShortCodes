...
%% .wav file 
file_name   = 'Test_cases_SW_SPAR_additionnal'; % name of the .wav file
file_header = 'Irregular waves for ECN campaign SOFTWIND SPAR'; % will appear in the client.exe windows
file_path  = strcat('SW_SPAR_waves_additionnal', filesep); % path relative to the current routine's path where the .wav file will be stored
%% Frequency spectrum
...
for m=1:N_spectra
    ...
    if write_phase_file == 1
        phase_file = strcat(file_path,'SW_SPAR_phase_m', num2str(m), '.mat');
