...
function [time2,out_wmk,out_eta,freq,out_FT_wmk,out_FT_eta] = probe2ocean_FP(filename_ocean,time,data,amp,type_wmk,f_min,f_max,Correc_2nd,Correc_free)
...
title = strcat('Time Reversal - First propagation');
...
%             warning('change to validate in Dalzell treatment...')
    ...
    %
    % Analysis
    %rnumb = nextpow2(clock_rate * T_d)+2; %+2 is to ensure that it is sufficiently long
...
%     plot(wm/(2*pi),angle(Fwm))
    ...
    %
    filename_export = [filename_ocean '_' num2str(round(100*amp(i1))) '%'];
    %
    ...
    ocean_write_summary_data(fid_sum, round(100*amp(i1)), wv);
    %N_2000 = export(wv, 'ocean', filename_export);
    N_2000 = export(wv, 'ocean_correction_TF', filename_export);
    %
    %% output of ocean file
    ocean_write_data_read(fid, name_short, round(100*amp(i1)), N_2000);
...
fprintf(fid,'begin\n');
for i1=1:length(amp)
    run = strcat('First Propagation, Amplitude = ', num2str(amp(i1)*100), ' %');
    ocean_write_run(fid, run, rnumb, round(100*amp(i1)));
end
fprintf(fid,'end;\n');
fclose(fid);
fclose(fid_sum);
...
%
X_A  = Fwm(1:n_fft/2)./amp(end); % keeping only frequencies between 0 and f_acq / 2
...
% %
% % test the use of specific routines: this gives exctly same results... OK
% out_eta2 = eval_eta(wv,time,0);
% out_wmk2 = eval_X(wv,time);
% 
% figure;
% plot(time,out_eta2,time2,out_eta,'--')
% 
% figure;
% plot(time,out_wmk2,time2,out_wmk,'--')
% figure(5);
% plot(time,data)
% hold on
