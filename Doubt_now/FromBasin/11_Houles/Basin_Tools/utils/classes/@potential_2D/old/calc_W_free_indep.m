function [W_evan] = calc_W_free_indep(pot_2D, x, z, n_evan)
% [W_evan] = calc_W_free_indep(pot_2D, x, z, time)
% @POTENTIAL_2D\CALC_W_FREE_INDEP evaluates the 2nd order free wave velocities at position x,z at time t for the potential_2D object
% Inputs: 
...
%         temp        = temp + pot_2D.sigma_n(n) * a_l_n_indep * Fmz * emikx;
        a_l_mn_indep = calc_a_l_mn_indep(omega(m), pot_2D.alpha_n(m,:), ...
            pot_2D.TF_n(m,:,1)+pot_2D.TF_n(m,:,2), pot_2D.sigma_n(n));
        temp        = temp + pot_2D.sigma_n(n) * a_l_mn_indep * Fmz * emikx;
    end
